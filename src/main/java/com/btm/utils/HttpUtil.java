...
public class HttpUtil {
     ...
     * @return
     */
    public static String httpPostMultipartFile(String url, Map<String, String> headers, MultipartFile file) throws Exception {
//        if (CollectionUtils.isEmpty(headers)) {
//            httpPostMultipartFile(url, file);
//        }
...
//                log.warn("Http POST 请求失败; [ errorCode = {}, url={}, param={}]", response.code(), url, json);
    ...
    public static <T> BaseResponse<T> doGet(String url, MultiValueMap<String, String> headers, Map<String, Object > urlParams, Class<T> tClass) {
        try {
            ...
            ResponseEntity<T> result = restTemplate.exchange(url, HttpMethod.GET, new HttpEntity<>(headers), tClass);

            if (result.getBody() != null) {
                log.info("Http Get: {} , Res：{}", url, JSONObject.toJSONString(result.getBody()));
            }
        ...
        return BaseResponse.serverError("请求失败！");
    }

    public static <T> BaseResponse<T> doPost(String url, MultiValueMap<String, String> headers, Object body, Class<T> tClass) {
        try {
            RestTemplate restTemplate = new RestTemplate();
            ...
            ResponseEntity<T> result = restTemplate.exchange(url, HttpMethod.POST, httpEntity, tClass);

            if (result.getBody() != null) {
                log.info("Http Post: {} , Res：{}", url, JSONObject.toJSONString(result.getBody()));
            }
